...
var applyOptions = []prompt.Suggest{
	...
	{Text: "--cascade", Description: "Only relevant during a prune or a force apply. If true, cascade the deletion of the resources managed by pruned or deleted resources (e.g. Pods created by a ReplicationController)."},
	{Text: "--dry-run", Description: "If true, only print the object that would be sent, without sending it."},
	{Text: "-f", Description: "Filename, directory, or URL to files that contains the configuration to apply"},
	{Text: "--filename", Description: "Filename, directory, or URL to files that contains the configuration to apply"},
	{Text: "--force", Description: "Delete and re-create the specified resource, when PATCH encounters conflict and has retried for 5 times."},
	{Text: "--grace-period", Description: "Only relevant during a prune or a force apply. Period of time in seconds given to pruned or deleted resources to terminate gracefully. Ignored if negative."},
	...
	{Text: "--prune", Description: "Automatically delete resource objects, including the uninitialized ones, that do not appear in the configs and are created by either apply or create --save-config. Should be used with either -l or --all."},
	...
	{Text: "--timeout", Description: "Only relevant during a force apply. The length of time to wait before giving up on a delete of the old resource, zero means determine a timeout from the size of the object. Any other values should contain a corresponding time unit (e.g. 1s, 2m, 3h)."},
